...
function ACL:createGroup ( name, permissions, creator, tag, tagColour )
	if ( name and type ( permissions ) == "table" ) then
		if ( not self.groups [ name ] ) then
			...
			cmd:Bind ( 2, ( creator or "System" ) )
			cmd:Bind ( 3, json.encode ( permissions ) )
			cmd:Bind ( 4, theDate )
			cmd:Bind ( 5, tostring ( name ) )
			cmd:Bind ( 6, "255, 0, 0" )
			cmd:Execute ( )
			self.groups [ name ] =
				{
...
function ACL:groupSetColor ( group, color )
	if ( group and color ) then
		if ( self.groups [ group ] ) then
			local cmd = SQL:Command ( "UPDATE acl_groups SET tagColour = ? WHERE name = ?" )
					cmd:Bind ( 1, color )
					cmd:Bind ( 2, group )
					cmd:Execute ( )
			return true
...
function ACL:groupSetTag ( group, tag )
	if ( group and tag ) then
		if ( self.groups [ group ] ) then
			local cmd = SQL:Command ( "UPDATE acl_groups SET tag = ? WHERE name = ?" )
					cmd:Bind ( 1, tag )
					cmd:Bind ( 2, group )
					cmd:Execute ( )
			return true
