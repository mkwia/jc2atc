...
local firstAdmin = "76561198056877306" -- CHANGE IT!
local msgColors =
	{
		[ "err" ] = Color ( 255, 0, 0 ),
		[ "info" ] = Color ( 0, 255, 0 ),
...
function Admin:executeAction ( args, player )
	if IsValid ( player, false ) then
		if ACL:hasObjectPermissionTo ( tostring ( player:GetSteamId ( ) ), args [ 1 ] ) then
			...
			elseif ( args [ 1 ] == "player.mute" ) then
				if IsValid ( args [ 2 ], false ) then
					if mute:isPlayerMuted ( args [ 2 ] ) then
						...
						if ( muteArgs.duration > 0 ) then
							local chatlengthdivider = 60
							if ( tostring ( args [ 5 ] ) == "Days" ) then
								muteArgs.duration = ( muteArgs.duration * 1440 )
								chatlengthdivider = 86400
							elseif ( tostring ( args [ 5 ] ) == "Hours" ) then
								muteArgs.duration = ( muteArgs.duration * 60 )
								chatlengthdivider = 3600
							end
							if mute:setPlayerMuted ( muteArgs, true ) then
								Chat:Broadcast ( args [ 2 ]:GetName ( ) .." was muted by ".. player:GetName ( ) .." for ".. tostring ( muteArgs.duration / chatlengthdivider ) .." ".. string.lower ( tostring ( args [ 5 ] ) ) .." ( ".. tostring ( muteArgs.reason ) .." )", Color ( 255, 0, 0 ) )
								self:requestInformation ( args [ 2 ], player )
							else
			...
			elseif ( args [ 1 ] == "player.warpto" ) then
				if IsValid ( args [ 2 ], false ) then
					if IsValid ( args [ 3 ], false ) then
						...
						player:Message ( "You warped ".. args [ 3 ]:GetName ( ) .." to ".. args [ 2 ]:GetName ( ), "info" )
			...
			elseif ( args [ 1 ] == "general.tab_adminchat" ) then
				for _, thePlayer in pairs ( self.canChat ) do
					...
					if thePlayer ~= player then
						Network:Send(thePlayer, "admin.promptChatMessage")
					end
				end
				...
				if ( args [ 2 ] ) then
					if ( args [ 3 ] ) then
						if ACL:groupRemoveObject ( args [ 2 ], args [ 3 ] ) then
							player:Message ( "You have successfully removed object ".. tostring ( args [ 3 ] ) .." from ACL group ".. tostring ( args [ 2 ] ) .."!", "err" )
							self:getACL ( nil, player )
						else
			...
			elseif ( args [ 1 ] == "acl.setcolor" ) then
				if ( args [ 2 ] ) then
					if ( args [ 3 ] ) then
						if ACL:groupSetColor ( args [ 2 ], args [ 3 ] ) then
							player:Message ( "You have successfully set tag colour ".. tostring ( args [ 3 ] ) .." in ACL group ".. tostring ( args [ 2 ] ) .."!", "err" )
							self:getACL ( nil, player )
						else
							player:Message ( "Failed to set colour!", "err" )
						end
					else
						player:Message ( "No colour was given.", "err" )
					end
				else
					player:Message ( "No group name was given.", "err" )
				end
			elseif ( args [ 1 ] == "acl.settag" ) then
				if ( args [ 2 ] ) then
					if ( args [ 3 ] ) then
						if ACL:groupSetTag ( args [ 2 ], args [ 3 ] ) then
							player:Message ( "You have successfully set tag ".. tostring ( args [ 3 ] ) .." in ACL group ".. tostring ( args [ 2 ] ) .."!", "err" )
							self:getACL ( nil, player )
						else
							player:Message ( "Failed to set tag!", "err" )
						end
					else
						player:Message ( "No tag was given.", "err" )
					end
				else
					player:Message ( "No group name was given.", "err" )
				end
			elseif ( args [ 1 ] == "module.load" ) then
				if ( args [ 2 ] == "module.reload" ) then
					if self:moduleExists ( tostring ( args [ 3 ] ) ) then
						if self:moduleIsValid ( tostring ( args [ 3 ] ) ) then
							...
							player:Message ( "No valid module found.", "err" )
						end
					else
					...
					if self:moduleExists ( tostring ( args [ 2 ] ) ) then
						if self:moduleIsValid ( tostring ( args [ 2 ] ) ) then
...
function Admin:moduleIsValid ( module )
