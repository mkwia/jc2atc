...
function Admin:__init ( )
	...
	self.reasons =
		{
			"RULE 1. Spamming.",
            "RULE 2. Respect the admins and mods.",
            "RULE 3. Impersonating a staff member.",
            "RULE 4. Respect players.",
            "RULE 5. Blocking, Ramming, Attacking Planes.",
            "RULE 6. PvP with random players.",
            "RULE 7. Racism, sexism, other abuse.",
			"Insulting/Flaming",
			"Cheating/Hacking",
			"Abusing bugs",
			"Advertising",
	...
	self.panel.tabPanel, self.tabs = GUI:TabControl ( { "Players", "ACL", "Bans", "Modules", "Server", "Admin Chat" }, Vector2 ( 0.0, 0.0 ), Vector2 ( 0.0, 0.0 ), self.panel.window )
	...
	self.panel.kill = GUI:Button ( "Kill", Vector2 ( 0.36, 0.09 ), Vector2 ( 0.06, 0.03 ), self.panel.playersTab, "player.kill" )
	...
	self.panel.shout = GUI:Button ( "Shout", Vector2 ( 0.423, 0.09 ), Vector2 ( 0.06, 0.03 ), self.panel.playersTab, "player.shout" )
	self.panel.shout:Subscribe ( "Press", self, self.showShoutWindow )

	self.panel.god = GUI:Button ( "Godmode", Vector2 ( 0.36, 0.62 ), Vector2 ( 0.123, 0.03 ) , self.panel.playersTab )
	self.panel.god:SetToggleable ( true )
	self.panel.god:Subscribe( "ToggleOn", self, function ( ) god:enable ( ) end )
	self.panel.god:Subscribe( "ToggleOff", self, function ( ) god:disable ( ) end )

	Network:Subscribe ( "updategodlist", function ( list ) god:updategodlist ( list ) end )

	Events:Subscribe ( "Render", function ( )

		local list = god:requestlist ( )

		if not list then return end

		if list [ LocalPlayer:GetSteamId ( ).string ] then
			local TextSize = 30
			local message = "GodMode Active"
			local textlength = Render:GetTextWidth ( message, TextSize )
			Render:DrawText ( Vector2 ( ( Render.Size.x - textlength - 74 ), 24 ), message, Color ( 0, 0, 0, 200 ), TextSize )
			Render:DrawText(Vector2 ( ( Render.Size.x - textlength - 75 ), 23 ), message, Color ( 255, 10, 255 ), TextSize )
			if god:godtimer ( ) then
				Game:FireEvent ( "ply.makeinvulnerable" )
				Network:Send ( "godHeal", LocalPlayer )
			end
		end
		for ply in Client:GetStreamedPlayers ( ) do
			if list [ ply:GetSteamId ( ).string ] then
				local color = ply:GetColor ( )
				color.a = 60
				local transf = Transform3 ( )
				transf:Translate ( ply:GetPosition ( ) )
				transf:Rotate ( ply:GetAngle ( ) )
				Render:SetTransform ( transf )
				local y = 2
				local yy = 0.5
				local x = 0.25
				local z = 0.25
				Render:FillTriangle(Vector3(0, y, 0), Vector3(x, y+yy, 0), Vector3(0, y+yy, z), color)
				Render:FillTriangle(Vector3(0, y, 0), Vector3(0, y+yy, z), Vector3(-x, y+yy, 0), color)
				Render:FillTriangle(Vector3(0, y, 0), Vector3(-x, y+yy, 0), Vector3(0, y+yy, -z), color)
				Render:FillTriangle(Vector3(0, y, 0), Vector3(0, y+yy, -z), Vector3(x, y+yy, 0), color)
				Render:FillTriangle(Vector3(x, y+yy, 0), Vector3(0, y+yy, z), Vector3(-x, y+yy, 0), color)
				Render:FillTriangle(Vector3(x, y+yy, 0), Vector3(0, y+yy, -z), Vector3(-x, y+yy, 0), color)
			end
		end
	...
	self.panel.chatScroll = GUI:ScrollControl ( Vector2 ( 0.0, 0.01 ), Vector2 ( 0.48, 0.63 ), self.panel.adminchatTab )
	self.panel.chatScroll:SetScrollable ( false, true )
	...
	self.panel.chatMessages = GUI:Label ( "", Vector2 ( 0.0, 0.011 ), Vector2 ( 0.47, 0 ), self.panel.chatScroll )
	...
	self.panel.aclCreateGroup = GUI:Button ( "Create group", Vector2 ( 0.21, 0.64 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclCreateGroup:Subscribe ( "Press", self, self.showACLCreateWindow )
	self.panel.aclDestroyGroup = GUI:Button ( "Destroy group", Vector2 ( 0.21, 0.68 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclDestroyGroup:Subscribe ( "Press", self, self.destroyACLGroup )
	self.panel.aclAddObject = GUI:Button ( "Add object", Vector2 ( 0.29, 0.64 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclAddObject:Subscribe ( "Press", self, self.showACLAddObjectWindow )
	self.panel.aclRemoveObject = GUI:Button ( "Remove object", Vector2 ( 0.29, 0.68 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclRemoveObject:Subscribe ( "Press", self, self.removeACLObject )
	self.panel.aclSetColor = GUI:Button ( "Set Colour", Vector2 ( 0.37, 0.64 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclSetColor:Subscribe ( "Press", self, self.showACLsetcolor )
	self.panel.aclSetTag = GUI:Button ( "Set Tag", Vector2 ( 0.37, 0.68 ), Vector2 ( 0.07, 0.03 ), self.panel.aclTab )
	self.panel.aclSetTag:Subscribe ( "Press", self, self.showACLsettag )

	self.permPanelChange.window = GUI:Window ( "ACL Permission", Vector2 ( 0.0, 0.0 ), Vector2 ( 0.15, 0.15 ) )
	...
	self.permPanelChange.label2 = GUI:Label ( "Has Permission:", Vector2 ( 0.03, 0.045 ), Vector2 ( 0.12, 0.03 ), self.permPanelChange.window )
	self.permPanelChange.value = GUI:CheckBox ( "", Vector2 ( 0.095, 0.0425 ), "", self.permPanelChange.window )
	self.permPanelChange.set = GUI:Button ( "Set permission", Vector2 ( 0.01, 0.075 ), Vector2 ( 0.12, 0.03 ), self.permPanelChange.window )
	...
	self.aclColorPanel.window = GUI:Window ( "Set Group Colour: ", Vector2 ( 0.0, 0.0 ), Vector2 ( 0.15, 0.15 ) )
	self.aclColorPanel.window:SetVisible ( false )
	GUI:Center ( self.aclColorPanel.window )
	GUI:Label ( "Colour '0, 0, 0': ", Vector2 ( 0.001, 0.001 ), Vector2 ( 0.1, 0.03 ), self.aclColorPanel.window )
	self.aclColorPanel.value = GUI:TextBox ( "", Vector2 ( 0.0, 0.03 ), Vector2 ( 0.14, 0.03 ), "text", self.aclColorPanel.window )
	self.aclColorPanel.add = GUI:Button ( "Update", Vector2 ( 0.0, 0.07 ), Vector2 ( 0.14, 0.03 ), self.aclColorPanel.window )
	self.aclColorPanel.add:Subscribe ( "Press", self, self.SetColor )

	self.aclTagPanel.window = GUI:Window ( "Set Group Tag: ", Vector2 ( 0.0, 0.0 ), Vector2 ( 0.15, 0.15 ) )
	self.aclTagPanel.window:SetVisible ( false )
	GUI:Center ( self.aclTagPanel.window )
	GUI:Label ( "Tag: ", Vector2 ( 0.001, 0.001 ), Vector2 ( 0.1, 0.03 ), self.aclTagPanel.window )
	self.aclTagPanel.value = GUI:TextBox ( "", Vector2 ( 0.0, 0.03 ), Vector2 ( 0.14, 0.03 ), "text", self.aclTagPanel.window )
	self.aclTagPanel.add = GUI:Button ( "Update", Vector2 ( 0.0, 0.07 ), Vector2 ( 0.14, 0.03 ), self.aclTagPanel.window )
	self.aclTagPanel.add:Subscribe ( "Press", self, self.SetTag )
	...
	self.vehColorPanel.tone1 = GUI:ColorPicker ( true, Vector2 ( 0.0, 0.0 ), Vector2 ( 0.3, 0.23 ), self.vehColorPanel.tabs [ "tone1" ].base )
	self.vehColorPanel.tone1Set = GUI:Button ( "Set colour", Vector2 ( 0.0, 0.24 ), Vector2 ( 0.282, 0.03 ), self.vehColorPanel.tabs [ "tone1" ].base )
	self.vehColorPanel.tone1Set:Subscribe ( "Press", self, self.setVehicleTone1Colour )
	self.vehColorPanel.tone2 = GUI:ColorPicker ( true, Vector2 ( 0.0, 0.0 ), Vector2 ( 0.3, 0.23 ), self.vehColorPanel.tabs [ "tone2" ].base )
	self.vehColorPanel.tone2Set = GUI:Button ( "Set colour", Vector2 ( 0.0, 0.24 ), Vector2 ( 0.282, 0.03 ), self.vehColorPanel.tabs [ "tone2" ].base )
	...
	Events:Subscribe ( "Render", self, self.RenderChatPrompt)
	Events:Subscribe ( "LocalPlayerEnterVehicle", self, self.GodVehicle)
	Events:Subscribe ( "LocalPlayerExitVehicle", self, self.UnGodVehicle)
	Events:Subscribe ( "ModuelUnload", self, self.Unload )
	-- Network events
	...
	Network:Subscribe ( "admin.promptChatMessage", self, self.promptChatMessage)
...
function Admin:onKeyPress ( args )
	...
	if ( args.key == VirtualKey.Return ) then
		if self:isActive ( ) then
			if self.panel.tabPanel:GetCurrentTab ( ):GetPage ( ) == self.tabs.adminchat.base then
				self.panel.chatMessage:Focus ( )
...
function Admin:addPlayerToList ( player )
	...
	item:SetTextColor(player:GetColor())
...
function Admin:onPlayerJoin ( args )
	...
	if LocalPlayer == args.player then god:disable ( ) end
end
...
function Admin:mutePlayer ( )
	if IsValid ( self.victim, false ) then
		...
		Network:Send ( "admin.executeAction", { "player.mute", self.victim, reason, ( duration * 60 ), durationMethod } )
		self.victim = false
		self.mutePanel.window:SetVisible ( false )
	else
		self:Message ( "No player selected.", "err" )
...
function Admin:warpPlayerTo ( )
	if IsValid ( self.victim, false ) then
		...
		if ( player ) then
			Network:Send ( "admin.executeAction", { "player.warpto", player, self.victim } )
			self.warpPanel.window:SetVisible ( false )
		else
...
function Admin:addChatMessage ( args )
	...
	if ( text == "" ) then
		self.panel.chatMessages:SetText ( "["..tostring ( os.date ( "%X" ) ).."] ".. args.msg )
	else
		self.panel.chatMessages:SetText ( text .."\n["..tostring ( os.date ( "%X" ) ).."] ".. args.msg )
	end
	self.panel.chatMessages:SizeToContents ( )
end

function Admin:promptChatMessage ( )
	if self.panel.tabPanel:GetCurrentTab ( ):GetPage ( ) == self.tabs.adminchat.base and self.active then return end
	self.chatprompttimer = Timer ( )
	self.chatprompttimer:Restart ( )
	self.prompt = true
end

function Admin:RenderChatPrompt ( )
	if IsValid ( self.chatprompttimer ) and self.prompt then
		if self.panel.tabPanel:GetCurrentTab ( ):GetPage ( ) == self.tabs.adminchat.base and self.active then
			self.prompt = false
		end
		local num = self.chatprompttimer:GetSeconds ( )
		local ceil = math.ceil ( num )
		--if ceil - num > 0.5 then
			local rs = Render.Size
			local string = "Admin Message!"
			local fontsize = 28
			local size = Render:GetTextSize ( string, fontsize ) /2
			Render:DrawText ( Vector2 ( rs.x * 0.9 - size.x, rs.y * 0.5 - size.y ), string, Color ( 255, 0, 0, 255 * ( ceil - num ) ), fontsize )
		--end
	end
end

function Admin:GodVehicle ( )
	local list = god:requestlist ( )
...
function Admin:onACLRightClick ( node )
	...
	local group = node:GetParent():GetParent():GetText()
	local value = node:GetDataString ( "value" )
	self.permPanelChange.window:SetVisible ( true )
	self.permPanelChange.label:SetText ( "Group: ".. tostring ( group ) .."\nPermission: ".. tostring ( perm ) )
	self.permPanelChange.label:SizeToContents ( )
	self.permPanelChange.value:SetChecked ( toboolean ( value ) )
end

function Admin:modifyACLPermission ( )
	local value = tostring ( self.permPanelChange.value:GetChecked ( ) )
	local groupnperm = self.permPanelChange.label:GetText ( ):split ( "\n" )
	local group = groupnperm[1]:gsub ( "Group: ", "" )
	local perm = groupnperm[2]:gsub ( "Permission: ", "" )
...
function Admin:showACLsetcolor ( )
	local group = self.panel.aclTree:GetDataString ( "group" )
	if ( group and group ~= "" ) then
		self.aclColorPanel.window:SetTitle ( "Set group colour: ".. tostring ( group ) )
		self.aclColorPanel.window:SetDataString ( "group", group )
		self.aclColorPanel.window:SetVisible ( true )
	end
end

function Admin:showACLsettag ( )
	local group = self.panel.aclTree:GetDataString ( "group" )
	if ( group and group ~= "" ) then
		self.aclTagPanel.window:SetTitle ( "Set group tag: ".. tostring ( group ) )
		self.aclTagPanel.window:SetDataString ( "group", group )
		self.aclTagPanel.window:SetVisible ( true )
	end
end
...
function Admin:SetColor ( )
	local group = self.aclColorPanel.window:GetDataString ( "group" )
	if ( group and group ~= "" ) then
		local steamID = self.aclColorPanel.value:GetText ( )
		if ( steamID ~= "" ) then
			Network:Send ( "admin.executeAction", { "acl.setcolor", group, steamID } )
			self.aclColorPanel.window:SetVisible ( false )
		else
			self:Message ( "Add a colour 'r, g, b'.", "err" )
		end
	end
end

function Admin:SetTag ( )
	local group = self.aclTagPanel.window:GetDataString ( "group" )
	if ( group and group ~= "" ) then
		local steamID = self.aclTagPanel.value:GetText ( )
		if ( steamID ~= "" ) then
			Network:Send ( "admin.executeAction", { "acl.settag", group, steamID } )
			self.aclTagPanel.window:SetVisible ( false )
		else
			self:Message ( "Add a tag.", "err" )
		end
	end
end
...
function Admin:Unload()
	if not IsValid ( self.panel ) then return end
	self.vehColorPanel.tone2Set:Remove()
	self.vehColorPanel.tone2:Remove()
	self.vehColorPanel.tone1Set:Remove()
	self.vehColorPanel.tone1:Remove()
	for _, i in pairs(self.vehColorPanel.tabs) do
		i:Remove()
	end
	self.vehColorPanel.tabPanel:Remove()
	self.vehColorPanel.window:Remove()

	self.panel.modulesRefresh:Remove()
	self.panel.moduleUnload:Remove()
	self.panel.moduleReload:Remove()
	self.panel.moduleLoad:Remove()
	self.panel.modulesLog:Remove()
	self.panel.modulesSearch:Remove()
	self.panel.modulesList:Remove()

	self.aclTagPanel.add:Remove()
	self.aclTagPanel.value:Remove()
	self.aclTagPanel.window:Remove()

	self.aclColorPanel.add:Remove()
	self.aclColorPanel.value:Remove()
	self.aclColorPanel.window:Remove()

	self.aclObjectPanel.add:Remove()
	self.aclObjectPanel.value:Remove()
	self.aclObjectPanel.window:Remove()

	self.aclCreatePanel.create:Remove()
	self.aclCreatePanel.selectAll:Remove()
	self.aclCreatePanel.select:Remove()
	self.aclCreatePanel.permissions:Remove()
	self.aclCreatePanel.name:Remove()
	self.aclCreatePanel.window:Remove()

	self.permPanelChange.set:Remove()
	self.permPanelChange.value:Remove()
	self.permPanelChange.label:Remove()
	self.permPanelChange.window:Remove()

	self.panel.aclSetTag:Remove()
	self.panel.aclSetColor:Remove()
	self.panel.aclRemoveObject:Remove()
	self.panel.aclAddObject:Remove()
	self.panel.aclDestroyGroup:Remove()
	self.panel.aclCreateGroup:Remove()
	self.panel.aclDataLabel:Remove()
	self.panel.aclTree:Remove()

	self.panel.clearMessage:Remove()
	self.panel.sendMessage:Remove()
	self.panel.chatMessage:Remove()
	self.panel.chatScroll:Remove()
	self.panel.chatMessages:Remove()

	self.shoutPanel.shout:Remove()
	self.shoutPanel.message:Remove()
	self.shoutPanel.window:Remove()

	self.manualBanPanel.ban:Remove()
	self.manualBanPanel.durationBox:Remove()
	self.manualBanPanel.duration:Remove()
	self.manualBanPanel.reasonsBox:Remove()
	self.manualBanPanel.reasonEdit:Remove()
	self.manualBanPanel.reasonCheck:Remove()
	self.manualBanPanel.steamID:Remove()
	self.manualBanPanel.window:Remove()

	self.panel.banRefresh:Remove()
	self.panel.banAdd:Remove()
	self.panel.banRemove:Remove()
	self.panel.banReason:Remove()
	self.panel.banReasonScroll:Remove()
	self.panel.banResponsibleSteam:Remove()
	self.panel.banResponsible:Remove()
	self.panel.banDate:Remove()
	self.panel.banDuration:Remove()
	self.panel.banName:Remove()
	self.panel.banSteamID:Remove()
	self.panel.bansSearch:Remove()
	self.panel.bansList:Remove()

	self.warpPanel.warp:Remove()
	self.warpPanel.list:Remove()
	self.warpPanel.search:Remove()
	self.warpPanel.window:Remove()

	self.mutePanel.mute:Remove()
	self.mutePanel.durationBox:Remove()
	self.mutePanel.duration:Remove()
	self.mutePanel.durationLabel:Remove()
	self.mutePanel.reasonsBox:Remove()
	self.mutePanel.reasonEdit:Remove()
	self.mutePanel.reasonCheck:Remove()
	self.mutePanel.reasonLabel:Remove()
	self.mutePanel.window:Remove()

	self.kickPanel.kick:Remove()
	self.kickPanel.reasonsBox:Remove()
	self.kickPanel.reasonEdit:Remove()
	self.kickPanel.reasonCheck:Remove()
	self.kickPanel.reasonLabel:Remove()
	self.kickPanel.window:Remove()

	self.banPanel.ban:Remove()
	self.banPanel.durationBox:Remove()
	self.banPanel.duration:Remove()
	self.banPanel.durationLabel:Remove()
	self.banPanel.reasonsBox:Remove()
	self.banPanel.reasonEdit:Remove()
	self.banPanel.reasonCheck:Remove()
	self.banPanel.reasonLabel:Remove()
	self.banPanel.window:Remove()

	self.panel.setTimeStep:Remove()
	self.panel.serverTimeStepField:Remove()
	self.panel.serverTimeStep:Remove()
	self.panel.setWeatherSeverity:Remove()
	self.panel.serverWeatherField:Remove()
	self.panel.serverWeather:Remove()
	self.panel.setGameTime:Remove()
	self.panel.serverGameTimeField:Remove()	
	self.panel.serverGameTime:Remove()
	self.panel.serverTime:Remove()
	self.panel.serverSpawnPosition:Remove()
	self.panel.serverDescription:Remove()
	self.panel.serverPlayers:Remove()
	self.panel.serverName:Remove()

	self.panel.god:Remove()

	self.panel.shout:Remove()
	self.panel.giveWeapon:Remove()
	self.panel.weaponSlotMenu:Remove()
	self.panel.weaponMenu:Remove()
	self.panel.setVehicleColour:Remove()
	self.panel.repairVehicle:Remove()
	self.panel.destroyVehicle:Remove()
	self.panel.giveVehicle:Remove()
	self.panel.vehicleTemplateMenu:Remove()

	self.panel.vehicleMenu:Remove()
	self.panel.warpPlayerTo:Remove()
	self.panel.spectate:Remove()
	self.panel.warpTo:Remove()
	self.panel.giveMoney:Remove()
	self.panel.setMoney:Remove()
	self.panel.setModel:Remove()
	self.panel.setHealth:Remove()
	self.panel.valueField:Remove()
	self.panel.kill:Remove()
	self.panel.freeze:Remove()
	self.panel.mute:Remove()
	self.panel.kick:Remove()
	self.panel.bin:Remove()

	self.panel.playerVehicleHealth:Remove()
	self.panel.playerVehicle:Remove()
	self.panel.playerWeaponAmmo:Remove()
	self.panel.playerWeapon:Remove()
	self.panel.playerWorld:Remove()
	self.panel.playerModel:Remove()
	self.panel.playerAngle:Remove()
	self.panel.playerPosition:Remove()
	self.panel.playerMoney:Remove()
	self.panel.playerHealth:Remove()
	self.panel.playerGroups:Remove()
	self.panel.playerPing:Remove()
	self.panel.playerIP:Remove()
	self.panel.playerSteamID:Remove()
	self.panel.playerName:Remove()
	self.panel.playersSearch:Remove()
	self.panel.playersList:Remove()

	self.panel.adminchatTab:Remove()
	self.panel.serverTab:Remove()
	self.panel.modulesTab:Remove()
	self.panel.bansTab:Remove()
	self.panel.aclTab:Remove()
	self.panel.playersTab:Remove()
	self.panel.tabPanel:Remove()

	self.panel.window:Remove()
end
