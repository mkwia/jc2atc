...
function VehicleBuyMenuEntry:GetName()
	...
	if self.name ~= nil and self.name ~= "" then
		DisplayName = modelName .. "            " .. self.name 										-- EDIT ME (NEED TO ADD A NEW COLUMN)
	end
...
function VehicleBuyMenuEntry:GetGameName()
	local modelName = Vehicle.GetNameByModelId( self.model_id )
	local DisplayName = modelName
	if self.name ~= nil and self.name ~= "" then
		DisplayName = modelName .. "            " .. self.name 										-- EDIT ME (NEED TO ADD A NEW COLUMN)
...
--				VehicleBuyMenuEntry( 37, 0 ),
			...
			["Plane"] = {
				VehicleBuyMenuEntry( 59, 0, nil, nil, "(Extra EA300)" ),
				-- ^ Peek Airhawk 225
				VehicleBuyMenuEntry( 81, 0, nil, nil, "(Bede BD-5)" ),
				-- ^ Pell Silverbolt 6
				VehicleBuyMenuEntry( 51, 0, nil, nil, "(HA-420 HondaJet)" ),
				-- ^ Cassius 192
				VehicleBuyMenuEntry( 30, 0, nil, nil, "(AV-8B Harrier II)" ),
				-- ^ Si-47 Leopard
				...
				-- ^ Si-47 Leopard
				VehicleBuyMenuEntry( 34, 0, nil, nil, "(Sukhoi Su-47)" ),
				-- ^ G9 Eclipse
				VehicleBuyMenuEntry( 39, 0, nil, nil, "(Boeing 737)" ),
				-- ^ Aeroliner 474
				VehicleBuyMenuEntry( 85, 0, nil, nil, "(Boeing C-17 Globemaster III)" ),
				-- ^ Bering I-86DP
			...
			["DLC"] = {
				VehicleBuyMenuEntry( 75, 0, nil, nil, "", 5 ),
				--^ Tuk Tuk Boom Boom
				VehicleBuyMenuEntry( 58, 0, nil, nil, "", 5 ),
				--^ Chevalier Classic
				VehicleBuyMenuEntry( 82, 0, nil, nil, "", 5 ),
				--^ Chevalier Ice Breaker
				VehicleBuyMenuEntry( 20, 0, nil, nil, "", 5 ),
				--^ Monster Truck
				VehicleBuyMenuEntry( 53, 0, nil, nil, "", 5 ),
				--^ Agency Hovercraft
				VehicleBuyMenuEntry( 24, 0, nil, nil, "", 5 ),
				--^ F-33 DragonFly
			}
        },

        [self.types.Weapon] = {
            { "One-Handed", --[["Left Hand" ,]] "Two-Handed" },
            ["One-Handed"] = {
--                WeaponBuyMenuEntry( Weapon.Handgun, Price, Slot(Left: 0, Right: 1, Primary: 2), "NAME", RequiredRankNumber ),
				...
				WeaponBuyMenuEntry( Weapon.SignatureGun, 0, 1, "Rico's Signature Gun - DLC", 5 ),
            ...
            ["Two-Handed"] = {
				...
				WeaponBuyMenuEntry( Weapon.SignatureGun, 0, 2, "Rico's Signature Gun - DLC", 5 ),
				WeaponBuyMenuEntry( Weapon.Airzooka, 0, 2, "Air Propulsion Gun - DLC", 5 ),
				WeaponBuyMenuEntry( Weapon.ClusterBombLauncher, 0, 2, "Cluster Bomb Launcher - DLC", 5 ),
				WeaponBuyMenuEntry( Weapon.MultiTargetRocketLauncher, 0, 2, "Multi-target Rocket Launcher - DLC", 5 ),
				WeaponBuyMenuEntry( Weapon.QuadRocketLauncher, 0, 2, "Quad Rocket Launcher - DLC", 5 ),
				WeaponBuyMenuEntry( Weapon.AlphaDLCWeapon, 0, 2, "Bull's Eye Assault Rifle - DLC", 5 ),
            }
        },
