...
function BuyMenu:__init()
	...
	VIPRank			=	5
	VIPPlayers	=	{
					"24276561198108894136",
                    "STEAM_0:1:45324628", -- Tally
                    "STEAM_0:1:57084799", -- HenrikX
                    "STEAM_0:1:52108498", -- Happy
                    "STEAM_0:0:110953637" -- Lolbees
					}
    query = SQL:Query("SELECT * FROM donators")
    result = query:Execute()
    for _, p in ipairs(result) do
        table.insert(VIPPlayers, p.steam_id)
    end
    self.ammo_counts            = {
...
function BuyMenu:ColorChanged( args, sender )
    ...
    if IsValid(veh) then
        if self.vehicles[sender:GetId()] ~= nil or self.vehicles2[sender:GetId()] ~= nil then
...
function BuyMenu:PlayerFired( args, player )
   ...
   --[[ if  self:IsInHotspot( player:GetPosition() ) and 
    ...
    if success and category_id == self.types.Vehicle then
        ...
        local str = string.format("You have purchased a %s!", item:GetGameName())
		
        player:SendSuccessMessage( str )
    elseif success and category_id ~= self.types.Vehicle then
    	player:SetMoney( player:GetMoney() - item:GetPrice() )
        ...
        local str = string.format("You have purchased a %s!", item:GetName())
...
function BuyMenu:BuyVehicle( player, item, tone1, tone2 )
    ...
    if player:GetValue("Bounty") ~= nil then
        return false, "You cannot purchase a vehicle while you have a bounty!"
    end
	...
	if item:GetRank() ~= nil then
		...
		if VIPRank >= RequiredRank then
			 if self:IsVIPPlayer(player) then
				...
				return false, "You must be a Donator to buy this."
			end
		end
	end
	
    if player:GetState() == PlayerState.InVehiclePassenger or player:GetValue("inextraseat") then
        return false, "You cannot purchase a vehicle while in the passenger seat!"
    end

    --if player:GetLinearVelocity():Length() > 0.5 then
    --    return false, "You cannot purchase a vehicle while moving!"
    --end
...
function BuyMenu:ExecuteVehicle( player, item, tone1, tone2 )
    ...
    if not self:IsVIPPlayer(player) then
        ...
        if args.model_id == 65 then
            args.position = args.position + Vector3(0, 2.5, 0)
        end
        ...
        Events:Fire("BuyVehicle", v)
        --player:EnterVehicle( v, VehicleSeat.Driver )

        return true, ""
    else
        if IsValid( self.vehicles2[ player:GetId() ] ) and IsValid( self.vehicles[ player:GetId() ] ) then
                self.vehicles[ player:GetId() ]:Remove()
                self.vehicles[ player:GetId() ] = nil
                self.vehicles[ player:GetId() ] = self.vehicles2[ player:GetId() ]
                local args = {}
                args.model_id           = item:GetModelId()
                ...
                local v2 = Vehicle.Create( args )
                self.vehicles2[ player:GetId() ] = v2

                v2:SetUnoccupiedRespawnTime( nil )
                v2:SetDeathRemove(true)
                Events:Fire("BuyVehicle", v2)
                --player:EnterVehicle( v2, VehicleSeat.Driver )

                local vehiclenumber = 1

                return true, ""
        elseif not IsValid( self.vehicles[ player:GetId() ] ) then
            local args = {}
            args.model_id           = item:GetModelId()
            ...
            return true, ""
        elseif not IsValid( self.vehicles2[ player:GetId() ] ) then
            local args = {}
            args.model_id           = item:GetModelId()
