...
{
    --[[
    Internally, JC2-MP splits the world of Panau (32768m x 32768m) into cells.
    These cells are used to determine whether further distance calculations
    should be run on objects. Each object is placed within a 3x3 grid of cells,
    and the distance between an object and other objects is calculated, given
    the other objects are within that 3x3 grid.

    This distance is then compared with the stream distance. If it's less than
    the stream distance, then the object will be streamed in.

    In summary:
    The cells are used to determine which objects should have calculations
    run on them
    The stream distance is used to determine whether two objects can see
    each other, *given* that they are within a 3x3 grid of cells.

    If you want to increase the distance players and vehicles are visible from,
    you will have to increase both of these values!
    --]]

    -- The default distance before players are streamed out.
    -- Default value (in metres): 500
    PlayerStreamDistance        = 2000,

    -- The default distance before vehicles are streamed out.
    -- Default value (in metres): 500
    VehicleStreamDistance       = 3000,
